...
namespace CoPilot.ORM.Mapping
{
    public class TableMapEntry
    {
        ...
        {
            var rel = MemberToRelationshipMappings.SingleOrDefault(r => r.Value.Equals(relationship) && r.Key.DeclaringClassType.IsAssignableFrom(EntityType));
        ...
        public DbRelationship GetRelationshipByMember(ClassMemberInfo member)
        {
            var m = MemberToRelationshipMappings.SingleOrDefault(
                    r => r.Key.Name == member.Name && member.DeclaringClassType.IsAssignableFrom(r.Key.DeclaringClassType));
        ...
        {
            return col.MappedMembers.SingleOrDefault(r => r.DeclaringClassType.IsAssignableFrom(EntityType));
        }
        ...
        public Dictionary<DbColumn, ClassMemberInfo> GetMappedColumns()
        {
            ...
            var mappedColumns = allColumns
                .SelectMany(r => r.MappedMembers
                    .Where(m => m.DeclaringClassType.IsAssignableFrom(EntityType)), (column, info) => new {column, info })
