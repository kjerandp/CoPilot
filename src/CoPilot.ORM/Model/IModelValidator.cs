    ...
    public class SimpleModelValidator : IModelValidator
    {
        public bool Validate(IDb db)
        {
            ...
            foreach (var dbTable in db.Model.Tables)
            {
                ...
                try
                {
                    var sql = $"select top 1 {string.Join(",", dbTable.Columns.Select(r => r.ColumnName))} from {dbTable};select top 1 * from {dbTable}";
                    var res = db.Query(sql, null);
                    if (res.RecordSets.Last().Records.Length == 0)
                    {
                        Console.WriteLine(FormatMessage("> No data in table!", "No errors, but unable to validate columns and data types."));
                        Console.WriteLine();
                        continue;
                    }
                    ...
                    for (var f = 0; f < fieldNames.Length; f++)
                    {
                        ...
                        var modelCol = dbTable.Columns.FirstOrDefault(r => r.ColumnName.Equals(field.Name, StringComparison.OrdinalIgnoreCase));

                        if (modelCol == null)
                        {
                            Console.WriteLine(FormatMessage($"> '{field.Name}'", $"The column '{field.Name}' is not mapped."));
                        }
