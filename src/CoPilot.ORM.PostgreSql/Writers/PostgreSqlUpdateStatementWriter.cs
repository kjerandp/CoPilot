...
{
    public class PostgreSqlUpdateStatementWriter : IUpdateStatementWriter
    {
        private readonly PostgreSqlProvider _provider;

        public PostgreSqlUpdateStatementWriter(PostgreSqlProvider provider)
        {
            _provider = provider;
        }
        ...
        public SqlStatement GetStatement(OperationContext ctx, ScriptOptions options = null)
        {
            ...
            foreach (var col in ctx.Columns.Keys)
            {
                ...
                if (ctx.Args.ContainsKey(param.Name))
                {
                    ...
                    {
                        colBlock.Add($"{(colBlock.ItemCount > 0 ? "," : "")}{Util.SanitizeName(col.ColumnName)} = {valueString}");
                    }
                }
                else
                {
            ...
            statement.Script.Add($"UPDATE {Util.SanitizeName(ctx.Node.Table.TableName)} SET");
